        ...
        *,
        ...
        if use_uvloop and loop_factory is None:
            try:
                ...
                raise RuntimeError(
                    "uvloop requested via use_uvloop=True, but the 'uvloop' package is not installed"
                )
    ...
    def run(
        ...
        if loop_factory is None and options.get("use_uvloop", False):
            try:
                ...
                raise RuntimeError(
                    "uvloop requested via options['use_uvloop'], but the 'uvloop' package is not installed"
                )
            loop_factory = uvloop.new_event_loop
        debug = options.get("debug", None)
        loop_factory = options.get("loop_factory", None)

...
from trio import from_thread  # type: ignore[import]
import trio.lowlevel  # type: ignore[import]
try:
    from outcome import Error, Outcome, Value  # type: ignore[import]
except Exception:
    # Minimal fallback if the 'outcome' package is not available.
    from dataclasses import dataclass
    from typing import Any

    class Outcome:
        """Fallback base class for Outcome-like results."""

    @dataclass
    class Error(Outcome):
        """Wraps an exception for compatibility with outcome.Error."""
        exc: BaseException
    ...
    @dataclass
    class Value(Outcome):
        """Wraps a value for compatibility with outcome.Value."""
        value: Any
...
class TrioBackend(AsyncBackend):
    ...
    def run_async_from_thread(
        ...
        try:
            return from_thread.run(func, *args, trio_token=trio_token)
    ...
    def run_sync_from_thread(
        ...
        trio_token = cast("trio.lowlevel.TrioToken | None", token)
        try:
            return from_thread.run_sync(func, *args, trio_token=trio_token)
        except trio.RunFinishedError:
            raise RunFinishedError from None
